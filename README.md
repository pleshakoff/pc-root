...
#### Взаимодействие с другими сервисами     
 ...
 4. При обновлении данных родителя отправляет сообщение в брокер, из которого они считываются сервисом [User cache](#UserCache)
   и данные кэша по измененному пользователю сбрасываются
)  
...
3) Сообщение об изменении данных пользователя с последующем сбросом кэша 
4) Сообщение сервисам-подписчикам об изменении состава группы (удаление, доабавение ученика) НЕ РЕАЛИЗОВАНО 
5) Сообщение об удалении группы. НЕ РЕАЛИЗОВАНО 
...
После этого можно посмотреть логи сервиса [Notifier](#Notifier) которыей берет соообщение из очереди,
