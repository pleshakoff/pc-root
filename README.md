...
Авторизация организована следующим образом. Неавторизованный пользователь может создать сессию. 
После установки соединения он должен подписаться на рассылку. При подписке в заголовке stomp сообщения,
указать токен. Если это не будет сделано, то сервер разорвет сессию(я пока поставил таймаут 2 секунды). 

На сервере реализовно следующим образом. При хэндшейке создается сэссия и помещается в список сессий,
