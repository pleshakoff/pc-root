...
Для каждого сервиса своя очередь, каждый подписчик должен читать свою очередь. 
Если добавится еще один сервис, которому нужен будет состав группы, надо будет переконфигурировать брокер
...
4) Одним из преимуществ Rafka считается то, что она более устойчива к нагрузкам и лучше масштабируется. 
