...
5. [Проверка домашнего задания .](#test)
...
у сервиса с кэшированными данными пользователя [User cache](#UserCache) (GET ​/users​/{id}) и отправляет письмо 
(в текущей версии просто пишет в лог) 
...
у сервиса с кэшированными данными пользователя [User cache](#UserCache) (GET ​/users​/{id}) и отправляет смс (в текущей версии просто пишет в лог) 
...
Так же при изменении данных пользователя в сервисе [Classroom](#Classroom), сервису кэша [User cache](#UserCache) отправляется сообщение 
...
Теперь видно что данные для первого пользователя были сброшены, а данные остальных берутся из кэша 
